...
if (!empty($_GET['Disciplina']) || !empty($_GET['Ano']) || !empty($_GET['Bimestre'])) {
    ...
    {
        $data = $_GET['search'];
        $sql = "SELECT * FROM $tabela WHERE id LIKE '%$data%' or UNIDADES_TEMÁTICAS LIKE '%$data%' or HABILIDADES LIKE '%$data%' or 'OBJETOS_DE_CONHECIMENTOS' LIKE '%$data%'ORDER BY id ASC";
    }
    else
    {
